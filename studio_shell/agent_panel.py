...
AVATAR_SRC_DIR = AVATAR_DIR / "_src"
...
AVATAR_EMOTIONS = frozenset({"idle", "thinking", "talking", "happy"})
AVATAR_MIN_THINKING_SECONDS = 1.2
AVATAR_TALKING_FRAME_MS = 120
AVATAR_TALKING_PAUSE_MS = 80


def _set_avatar_emotion(emotion: str) -> None:
    if emotion in AVATAR_EMOTIONS:
        st.session_state["avatar_emotion"] = emotion


def _avatar_source_path(emotion: str) -> Path:
    png_path = AVATAR_SRC_DIR / f"{emotion}.png"
    if png_path.is_file():
        return png_path
    return AVATAR_DIR / f"{emotion}.gif"


@lru_cache(maxsize=16)
def _read_avatar_file_base64(path_str: str) -> str:
    return base64.b64encode(Path(path_str).read_bytes()).decode("ascii")


@lru_cache(maxsize=32)
def _load_avatar_frame_rgba(emotion: str) -> bytes:
...
def _png_bytes_to_rgba_frame(data: bytes) -> Image.Image:
    with Image.open(io.BytesIO(data)) as image:
        return image.convert("RGBA")


def _rgba_to_gif_palette_frame(frame: Image.Image) -> Image.Image:
...
@lru_cache(maxsize=8)
def _build_talking_avatar_base64(phase: int) -> str:
    if phase % 2 == 0:
        frame_names = ("idle", "talking", "idle")
    else:
        frame_names = ("talking", "idle", "talking")

    rgba_frames = [_png_bytes_to_rgba_frame(_load_avatar_frame_rgba(name)) for name in frame_names]
    palette_frames = [_rgba_to_gif_palette_frame(frame) for frame in rgba_frames]
    durations = [AVATAR_TALKING_FRAME_MS, AVATAR_TALKING_FRAME_MS, AVATAR_TALKING_PAUSE_MS]
...
def _render_avatar_ui(container: Any | None = None) -> None:
    target = container or st
    emotion = str(st.session_state.get("avatar_emotion", "idle"))
    ...
    if not source_path.is_file() and not avatar_path.is_file():
        target.info(f"缺少 Avatar 檔案：`{emotion}`")
        return

    avatar_data_url = _avatar_data_url(emotion)
    target.markdown(
        (
            "<div style='width:160px'>"
            f"<img src='{avatar_data_url}' "
            f"alt='{emotion}' width='160' style='display:block' />"
            "</div>"
        ),
...
def _refresh_avatar_ui(container: Any | None = None, emotion: str | None = None) -> None:
    if emotion is not None:
        _set_avatar_emotion(emotion)
    _render_avatar_ui(container)


def _wait_for_min_thinking_duration(started_at: float | None) -> None:
    if started_at is None:
        return
    elapsed = time.perf_counter() - started_at
    remaining = AVATAR_MIN_THINKING_SECONDS - elapsed
    if remaining > 0:
...
def render_chat_panel(*, extra_context: str = "", page_name: str = "") -> None:
    ...
    if "avatar_emotion" not in st.session_state:
        st.session_state["avatar_emotion"] = "idle"
    ...
    _render_avatar_ui(avatar_container)
    _render_tts_settings_ui(settings_error=settings_error)
    ...
    if should_process_submission:
        ...
        with chat:
            ...
            with st.chat_message("assistant"):
                ...
                thinking_started_at: float | None = time.perf_counter()
                st.session_state["avatar_talking_phase"] = 0
                _refresh_avatar_ui(avatar_container, "thinking")
                ...
                def on_token(token: str) -> None:
                    if token and not answer_parts:
                        _wait_for_min_thinking_duration(thinking_started_at)
                    if token:
                        st.session_state["avatar_talking_phase"] = (
                            int(st.session_state.get("avatar_talking_phase", 0)) + 1
                        )
                        _refresh_avatar_ui(avatar_container, "talking")
                ...
                try:
                    ...
                    if not answer_parts:
                        _wait_for_min_thinking_duration(thinking_started_at)
                    st.session_state["avatar_talking_phase"] = 0
                    _refresh_avatar_ui(avatar_container, "idle")
