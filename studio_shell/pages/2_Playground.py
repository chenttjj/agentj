...
def render_main() -> str:
    ...
    metric_col, btn_col = st.columns([2, 2])
    metric_col.metric("Count", st.session_state.playground_count)
    
    btn_sub_col1, btn_sub_col2 = st.columns(2)
    if btn_sub_col1.button("+1", use_container_width=True):
        st.session_state.playground_count += 1
        st.rerun()
    if btn_sub_col2.button("-1", use_container_width=True):
        if st.session_state.playground_count > 0:
            st.session_state.playground_count -= 1
        st.rerun()
...
""",
    ...
    # 實作：仿照 Home Page，使用 format_extra_context 並回傳
...
""",
    ...
    # 練習 1 — 用右欄 Prompt 請 Agent 接上 Extra Context，再 return 摘要字串
    return extra
